...
public class TicketManager
{
    ...
    static Map<Integer, TicketPassenger> passengers = new HashMap<>(); // map of passenger ids to passengers

    // book ticket
    public void reserveTicket(TicketPassenger p, int berthInfo, String allottedBerth)
    {
        // assign the seat number and type of berth (L, U, M)
        p.seatNumber = berthInfo;
        p.allottedType = allottedBerth;
    ...
    // adding to RAC
    public void addToRAC(TicketPassenger p, int racInfo, String allottedRAC)
    {
        // assign seat number and type (RAC)
        p.seatNumber = racInfo;
        p.allottedType = allottedRAC;
        // add passenger to the map
        passengers.put(p.passengerId, p);
    ...
    // adding to WL
    public void addToWaitingList(TicketPassenger p, int waitingListInfo, String allottedWL)
    {
        // assign seat number and type (WL)
        p.seatNumber = waitingListInfo; 
        p.allottedType = allottedWL;
        // add passenger to the map
        passengers.put(p.passengerId, p);
    ...
    public void cancelTicket(int passengerId)
    {
        ...
        TicketPassenger p = passengers.get(passengerId);
        ...
        if (p.allottedType.equals("L")) 
        { 
            availableLowerBerths++;
            lowerBerthsPositions.add(positionBooked);
        }
        else if (p.allottedType.equals("M"))
        { 
            availableMiddleBerths++;
            middleBerthsPositions.add(positionBooked);
        }
        else if (p.allottedType.equals("U"))
        { 
        ...
        if (racList.size() > 0)
        {
            ...
            TicketPassenger passengerFromRAC = passengers.get(racList.poll());
            ...
            if (waitingList.size() > 0)
            {
                ...
                TicketPassenger passengerFromWaitingList = passengers.get(waitingList.poll());
                ...
                passengerFromWaitingList.allottedType = "RAC";
                racPositions.remove(0);
            ...
            TicketReservationSystem.reserveTicket(passengerFromRAC);
        }
    ...
    public void printPassengers()
    {
        ...
        for (TicketPassenger p : passengers.values())
        {
            System.out.println("PASSENGER ID " + p.passengerId );
            System.out.println(" Name " + p.passengerName );
            System.out.println(" Age " + p.passengerAge );
            System.out.println(" Status " + p.seatNumber + p.allottedType);
            System.out.println("--------------------------");
        }
    }
